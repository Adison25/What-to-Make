...
func fetchData() -> [PhotoModel]{
    ...
    var counterDatabase = 1 //I should definitly have a better approach but a manual counter is cheap and  easy
    var indexDatabase = 1
    var stringDatabase = "1"
    
    let docRef = database.document("recipes/\(stringDatabase)")
    docRef.getDocument { snapshot, error in
        guard let data = snapshot?.data(), error == nil else {
            return
        }
        guard let name = data["ingredients"] as? [String] else {
            return
        }
        print(name[0])
    }
...
//                   sourceURL: "https://leitesculinaria.com/83100/recipes-white-chocolate-cupcakes.html?utm_campaign=yummly&utm_medium=yummly&utm_source=yummly#recipe",
//                                photoFileName: "meal1"),
